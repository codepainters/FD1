...
IapResult_t FlashIap_PrepareSector(const Sector_t*  sector)
{
    ...
    uint32_t command[5] = { 50, sector->number, sector->number };
    __disable_irq();
...
IapResult_t FlashIap_WriteSector(const Sector_t* sector, uint8_t* buffer, size_t bufferSize)
{
    ...
    uint32_t command[5] = { 51, (uint32_t) sector->address, (uint32_t) buffer, bufferSize, CPU_CLK };
    __disable_irq();
    IAPEntry(command, result);
    __enable_irq();
...
IapResult_t FlashIap_VerifySector(const Sector_t* sector, uint8_t* buffer, size_t bufferSize)
{
    ...
    uint32_t command[5] = { 56, (uint32_t) sector->address, (uint32_t) buffer, bufferSize };
    __disable_irq();
    IAPEntry(command, result);
    __enable_irq();
